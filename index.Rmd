...
-  The anticipated completion time is approximately eight hours.
...
**Role of Video Introductions.** There are five chapters in this tutorial that summarize the foundations. Each chapter is subdivided into several sections. At the beginning of each section is a short video, typically 6-8 minutes, that summarizes the section key learning outcomes. Following the video, you can see more details about the underlying `R` code for the analysis presented in the video. 
...
-  When you view the .pdf output for the overheads with the browser FireFox, the default is for the table of contents to appear. The fix for this is to click the icon in the upper left-hand corner. For other browsers we tested (Edge and Chrome), on the .pdf appears. You will have to scroll down to the appropriate page to see the desired output.
-  The graphical displays upon completion of the tutorial exercises appear only in the narrow window of the code and do not look good. This is a limitation of the *Datacamp* platform. For a more professional looking graph, you can simply copy the code and paste it into your own version of `R` to enhance its appearance.
