...
\[
\Pr(X = x) = \frac{(x+n-1)!}{(n-1)!x!}p^n(1-p)^x, \hspace{5mm} x=0, 1, ...
\]

This matches the definition in the textbook by equating $x=k$, probability $p = \frac{1}{1+\beta}$, and size $r=n$.  It is very important to carefully consider the parameterization of any distribution you use in `R`.
...
-  Generate PP plots for the positive loss amount `y` using Gaussian and gamma. Based on PP plots, which parametric fitted model is closer to the empirical distribution?
...
success_msg("Good job! QQ plot compares two fitted models through their quantiles. Again, the gamma is better fit.")
```
