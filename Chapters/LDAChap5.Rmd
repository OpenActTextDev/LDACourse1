...
In this assignment, we will fit a collective risk model for an actual portfolio of insurance contracts, **LGPIF** data with `R`. We use *negative binomial* and *Pareto* distributions for the frequency and severity components, respectively.
...
freq_lik <- function(parm) {
  r    <- parm[1]
  beta <- parm[2]
  lik  <- -sum(dnbinom(N, size=_, prob=1/(1+??), log=TRUE))
  return(lik)
}
init.parm.freq <- c(mean(N)/(var(N)/mean(N)-1), var(N)/mean(N)-1) # initial estimates by method of moments
freq_mod       <- ??(par=init.parm.freq, fn=??) # Maximum likelihood estimation for the frequency model
...
sev_mod       <- ??(par=??, fn=sev_lik, method="L-BFGS-B") # Maximum likelihood estimation for the severity model
...
freq <- expression(data =  ??(size=??, prob=1/(1+??)))  # random samples from the fitted frequency model
sev  <- expression(data =  ??(shape=??, scale=??)) # random samples from the fitted severity model
...
comploss_mod       <- optim(par=??,
                            fn=??, method="L-BFGS-B") # Maximum likelihood estimation for the compound loss model
...
plot(qtweedie(pct, mu=??, phi=??, power=p.est) ,quantile(InsampleTown$y, probs=pct), xlab="Theoretical Quantile", ylab="Empirical Quantile", xlim=c(0,50000), ylim=c(0,50000))
abline(0,1)
...
-  **Chapter Maintainers**. Please contact ??? and/or Jed <jfrees@bus.wisc.edu> for chapter comments and suggested improvments.
