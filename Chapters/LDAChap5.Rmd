...
freq_mod       <- ??_(par=init.parm.freq, fn=??) # Maximum likelihood estimation for the frequency model
...
sev_mod       <- ??_(par=??, fn=sev_lik, method="L-BFGS-B") # Maximum likelihood estimation for the severity model
...
freq <- expression(data =  ????(size=??, prob=1/(1+??_)))  # random samples from the fitted frequency model
