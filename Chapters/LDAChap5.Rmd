...
-  Although not needed to go through the tutorials, some users may wish to download the overheads that the videos are based on. <button download><a href="https://raw.githubusercontent.com/OpenActTextDev/LDACourse1/main/LDA1.Overheads/LDA1.Chap5.pdf">Download Chapter Five overheads as a .pdf file.</a></button>
...
### Exercise. Individual Risk Model I

**Assignment Text**
  
In this assignment, we will revisit [Example 5.2.1](https://openacttexts.github.io/Loss-Data-Analytics/C-AggLossModels.html) in the textbook and generate using *simulation* in `R`. Even though we will `officially' start to work with simulation since Chapter 6, this exercise provides you an opportunity to taste a flavor of simulation with an example.
...
Note that number of policies and probability of claim per policy correspond to the size and incidence probability in a binomial distribution for a claim frequency random variable, respectively.
```
...
success_msg("Good job! This exercise is basically a recapitulation of Chapter 2 but still quite important as one of the building blocks of Individual Risk Models.")
```
...
In this assignment, we will revisit [Example 5.2.1](https://openacttexts.github.io/Loss-Data-Analytics/C-AggLossModels.html) in the textbook and reproduce the theoretical results for moments calculation in individual risk models using *simulation* in `R`. We use the simulated frequencies for each group of policies in Exercise 5.5.2.1 and continue to simulate compound losses in an individual risk model as elaborated in [Section 5.4.2](https://openacttexts.github.io/Loss-Data-Analytics/C-AggLossModels.html#computing-the-aggregate-claims-distribution).
...
- Initialize an empty vector of length $r$ to store $r$ observations of simulated $S$. In each simulation,  calculate $j$th simulated aggregate loss and store it in $j$th component of $S$ for $j=1, \ldots, R$.
-  Compute mean and variance of $S$ by simulation using [mean()](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/mean) and [var()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/cor), and compare these values to their corresponding theoretical values, 2,800 and 600,467, respectively.
...
for(j in 1:r) { 
  x1_j <- runif(n1[j], min=m1, max=__)
  x2_j <- runif(_____, min=__, max=M2)
  S[j] <- sum(x1_j)+sum(___)
}
____(S) 
var(_)  
```
...
success_msg("Good job! Note that simulation is one of the most powerful techniques in loss data analytics so we believe this exercise helped you a bit to grow as an expert in loss data analytics.")
```
...
In this assignment, we will fit a collective risk model for an actual portfolio of insurance
contracts, **LGPIF** data with `R`. We use *negative binomial* and *Pareto* distributions for the frequency and severity components, respectively.

:::: {.blackbox }

**Instructions**

-  Use the function [optim()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/optim) to find the maximum likelihood estimates of the frequency and severity distributions.
- For the likelihood of frequency component, we use [dnbinom()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/NegBinomial). Note that we use the same parametrization as in the [textbook](https://openacttexts.github.io/Loss-Data-Analytics/C-Frequency-Modeling.html#S:basic-frequency-distributions) so that $\text{size}=r$ and $\text{prob}=1/(1+\beta)$.
- For the likelihood of severity component, we use [dlomax()](https://www.rdocumentation.org/packages/bayesmeta/versions/2.6/topics/dlomax). Note that we use the same parametrization as in the [textbook](https://openacttexts.github.io/Loss-Data-Analytics/C-Severity.html#S:ContinuousDistn) so that $\text{shape}=\alpha$ and $\text{scale}=\theta$.
::::
...
```{r ex="LDA5.5.3.1", type="sample-code", tut=TRUE, warning=FALSE, message=FALSE}
...
freq_lik <- function(parm) {
  r    <- parm[1]
  beta <- parm[2]
  lik  <- -sum(dnbinom(N, size=_, prob=1/(1+___), log=TRUE))
  return(lik)
}
init.parm.freq <- c(mean(N)/(var(N)/mean(N)-1), var(N)/mean(N)-1) # initial estimates by method of moments
freq_mod       <- _____(par=init.parm.freq, fn=___) # Maximum likelihood estimation for the frequency model

r.est    <- freq_mod$par[1]
beta.est <- freq_mod$par[2]

freq_table <- cbind(table(N)[1:4]/length(N), dnbinom(0:3, size=___, prob=1/(1+beta.est)))
freq_table <- rbind(freq_table, 1-colSums(freq_table))
rownames(freq_table)[5] <- "4+"
colnames(freq_table)    <- c("Empirical", "Theoretical")
...
sev_lik <- function(parm) {
  alpha <-  parm[1]
  theta <-  parm[2]
  lik   <- -sum(dlomax(Xbar, shape=___, scale=___, log=TRUE))
  return(lik)
}
init.parm.sev <- c( 2/(1-mean(Xbar)^2/var(Xbar))  , mean(Xbar)*(2/(1-mean(Xbar)^2/var(Xbar))-1) ) # initial estimates by method of moments
sev_mod       <- _____(par=___, fn=sev_lik, method="L-BFGS-B") # Maximum likelihood estimation for the severity model
...
```{r ex="LDA5.5.3.1", type="solution", tut=TRUE, warning=FALSE, message=FALSE}
InsampleTown <- subset(Insample, TypeTown==1)
...
success_msg("Cheers! Note that we usually add covariates in the frequency and severity models for risk classification purpose in practice.")
```
...
In this assignment, we will analyze aggregate loss distribution by combining fitted frequency and severity components. Since it is usually very hard to obtain a closed form of distribution function of the compound loss in a collective risk model, we rely on simulations to compute the aggregate loss distribution. 
...
- We use the fitted frequency and severity models using **LGPIF** data as we have done in Tutorial 5.3, with negative binomial and Pareto distributions, respectively.
- To generate random samples from the fitted frequency and severity models, we use [rnbinom()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/NegBinomial) and  [rlomax()](https://www.rdocumentation.org/packages/bayesmeta/versions/2.6/topics/dlomax). Note that these generated random samples are used as inputs to compute the aggregate claim amount distribution.
- We use [aggregateDist()](https://www.rdocumentation.org/packages/actuar/versions/3.1-1/topics/aggregateDist) to simulate compound losses that match the observed data upon the fitted frequency and severity models. It is an important task in order to visualize the distribution of compound losses and make subsequent decisions on ratemaking.
::::
...
freq <- expression(data =  ______(size=____, prob=1/(1+_____)))  # random samples from the fitted frequency model
sev  <- expression(data =  ______(shape=________, scale=_______)) # random samples from the fitted severity model
Fs   <- aggregateDist("simulation", nb.simul = 1000,
                      model.freq = ____, model.sev = ____)
plot(Fs)
...
freq <- expression(data =  rnbinom(size=r.est, prob=1/(1+beta.est))) # random samples from the fitted frequency model
sev  <- expression(data =  rlomax(shape=alpha.est, scale=theta.est)) # random samples from the fitted severity model
Fs   <- aggregateDist("simulation", nb.simul = 1000,
...
success_msg("Good job! This is the second time that we used the simulation technique. Note that it is usual to observe that an actual claims dataset may come with very few positive claims but also heavy tail behavior.")
```
...
In this assignment, we will fit a Tweedie model for a collective risk model using observed compound losses in LGPIF data with `R`. Unlike the previous tutorials that required seperate model fitting for the frequency and severity components, use of Tweedie distribution enables us to fit a collective risk model in one step.

:::: {.blackbox }

**Instructions**

- For the likelihood of the compound loss, We use [dtweedie()](https://www.rdocumentation.org/packages/tweedie/versions/2.3.3/topics/Tweedie), which is one of the built-in functions of `R` library [tweedie](https://cran.r-project.org/web/packages/tweedie/tweedie.pdf) .
...
- After estimating the parameters of the Tweedie model, plot the empiricial quantiles against the quantiles from the fitted model. Note that we generate the theoretical quantiles via [qtweedie()](https://www.rdocumentation.org/packages/tweedie/versions/2.3.3/topics/Tweedie) and the estimated Tweedie parameters.
::::
...
source("https://raw.githubusercontent.com/OpenActTextDev/LDACourse1/main/RCode/TweedieSourceCode.Rmd")
...
# The Tweedie compound loss model: S
S <- _____$y
comploss_lik <- function(parm) {
  mu    <- parm[1]
  phi   <- parm[2]
  p     <- parm[3]
  lik  <- -sum(  log(dtweedie(S, mu=mu, phi=phi, power=_)) )
  return(lik)
}
init.parm.comploss <- c( mean(S), 300, 1.5)    # initial estimates 
comploss_mod       <- optim(par=______,
                            fn=______, method="L-BFGS-B") # Maximum likelihood estimation for the compound loss model


mu.est  <- comploss_mod$par[1]
phi.est <- comploss_mod$___
p.est   <- comploss_mod$___

pct <- seq(0.01,0.99,0.01)
plot(qtweedie(pct, mu=____, phi=____, power=p.est) ,quantile(InsampleTown$y, probs=pct), xlab="Theoretical Quantile", ylab="Empirical Quantile", xlim=c(0,50000), ylim=c(0,50000))
abline(0,1)
...
init.parm.comploss <- c( mean(S), 300, 1.5)    # initial estimates 
comploss_mod       <- optim(par=init.parm.comploss,
                            fn=comploss_lik, method="CG") # Maximum likelihood estimation for the compound loss model
...
pct <- seq(0.01,0.99,0.01)
plot(qtweedie(pct, mu=mu.est, phi=phi.est, power=p.est) ,quantile(InsampleTown$y, probs=pct), xlab="Theoretical Quantile", ylab="Empirical Quantile", xlim=c(0,50000), ylim=c(0,50000))
abline(0,1)
...
success_msg("While Tweedie distribution enables us to fit a compound loss distribution in one step, actual claims dataset might not fit into the distribution especially when we observe heavy-tail behavior.")
```
