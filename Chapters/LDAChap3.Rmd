...
-  Although not needed to go through the tutorials, some users may wish to download the overheads that the videos are based on. <button download><a href="https://raw.githubusercontent.com/OpenActTexts/LDACourse1/main/LDA1.Overheads/LDA1.Chap3.pdf">Download Chapter Three overheads as a .pdf file.</a></button>
...
*  Use [mean()](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/mean) to calculate the first raw moment of `y1`, `y2`, `y3` and `y4`. Assign the values to `Vec_Means`.
...
*  There is no `base-R` function to calculate the coefficient of skewness. There are several approaches to calculate skewness if you explore the [help()](https://www.rdocumentation.org/packages/e1071/versions/1.7-4/topics/skewness) for the `type` argument of the `skewness` function from the [e1071](https://www.rdocumentation.org/packages/e1071/versions/1.7-7) package.  We will not use this package, but instead we will we calculate the equivalent of `type = 3` skewness using [sd()](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/sd) in the denominator.
:::: 
...
# Summaries
( Sum1 <- summary(anscombe$y1) ) #Example for the first data set in  Anscombe's quartet
( Sum2 <- summary(anscombe$??) ) 
( Sum3 <- summary(anscombe$??) ) 
( Sum4 <- summary(anscombe$??) ) 
# Vector of means
( VecMean <- c(mean(anscombe$y1), mean(anscombe$??), mean(anscombe$??),
  mean(anscombe$??)) )
# Vector of variances
( VecVar <- c(var(anscombe$??), var(anscombe$y2), var(anscombe$y3),
  var(anscombe$??)) )
# Skewness measure
...
# Summaries
( Sum1 <- summary(anscombe$y1) ) #Example for the first data set in  Anscombe's quartet
( Sum2 <- summary(anscombe$y2) ) 
( Sum3 <- summary(anscombe$y3) ) 
( Sum4 <- summary(anscombe$y4) ) 
# Vector of means
( VecMean <- c(mean(anscombe$y1), mean(anscombe$y2), mean(anscombe$y3),
  mean(anscombe$y4)) )
# Vector of variances
( VecVar <- c(var(anscombe$y1), var(anscombe$y2), var(anscombe$y3),
  var(anscombe$y4)) )
# Skewness measure
...
Error1a <- "Did you call the function `summary` for y1?"
Error1b <- "Did you call the function `summary` for y2?"
Error1c <- "Did you call the function `summary` for y3?"
Error1d <- "Did you call the function `summary` for y4?"

Error2 <- "Did you correctly specify the arguments of the `mean` function?"
Error3 <- "Did you correctly specify the arguments of the `variance` function?"
Error4 <- "Did you correctly specify the arguments of the skewness expression?" 

ex() %>% check_object("Sum1", undefined_msg = Error1a) %>% check_equal(incorrect_msg=Error1a)
ex() %>% check_object("Sum2", undefined_msg = Error1b) %>% check_equal(incorrect_msg=Error1b)
ex() %>% check_object("Sum3", undefined_msg = Error1c) %>% check_equal(incorrect_msg=Error1c)
ex() %>% check_object("Sum4", undefined_msg = Error1d) %>% check_equal(incorrect_msg=Error1d)

ex() %>% check_object("VecMean", undefined_msg = Error2) %>% check_equal(incorrect_msg=Error2)
ex() %>% check_object("VecVar", undefined_msg = Error3)  %>% check_equal(incorrect_msg=Error3)
ex() %>% check_object("y1_skew", undefined_msg = Error4) %>% check_equal(incorrect_msg=Error4)

success_msg("Excellent! Presenting the right summary statistics in a clean way is important for communication of results. It is also very useful as you are trying to make sense of a data set.")
```
...
The [boxplot()](https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/boxplot) function produces box-and-whisker plots. Specifically, the `boxplot()` function returns certain statistics and produces a plot as a side effect. Explore the documentation  to understand the values returned. In particular review the information returned by the `stats` attribute of this object.
...
*  Use the [quantile()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/quantile) function and replicate the "hinge" values for the 2008 box. (We ignore the values returned for the whiskers as they are not a function of the quantiles of the distribution.)    
...
Review [Section 3.1.2 of *Loss Data Analytics*](https://openacttexts.github.io/Loss-Data-Analytics/C-Severity.html#S:LS:Quantiles) to get an overview of quantiles.
...
Error1 <- "Did you call the function ` boxplot` for lny~ Year?"
Error2 <- "Did you call the function `quantile` for (lny_2008, 1/4)?"
Error3 <- "Did you call the function `median` for lny_2008?"
Error4 <- "Did you call the function `quantile` (lny_2008,3/4)?"

ex() %>% check_object("my_boxplot", undefined_msg = Error1) %>% check_equal(incorrect_msg=Error1)
ex() %>% check_function("quantile", not_called_msg=Error2) %>% check_result() %>% check_equal(incorrect_msg=Error2)
ex() %>% check_function("median", not_called_msg=Error3) %>% check_result() %>% check_equal(incorrect_msg=Error3)
ex() %>% check_function("quantile", not_called_msg=Error4) %>% check_result() %>% check_equal(incorrect_msg=Error4)

success_msg("Well done! Due to their simplicity, you will encounter boxplots extensively when trying to visualize more complex multivariate relationships. It is helpful to reinforce your understanding of this fundamental tool in this relatively simple context.")
```
...
In [Example 3.1.4 of *Loss Data Analytics*](https://openacttexts.github.io/Loss-Data-Analytics/C-Severity.html#moment-generating-function) you will find a discussion of the moment generating function for a gamma distribution,
...
```{r ex="LDA1.3.1.3", type="hint", tut=TRUE}
...
```{r ex="LDA1.3.1.3", type="pre-exercise-code", tut=TRUE}

```

```{r ex="LDA1.3.1.3", type="sample-code", tut=TRUE}
...
```{r ex="LDA1.3.1.3", type="solution", tut=TRUE}
...
```{r ex="LDA1.3.1.3", type="sct", tut=TRUE}
Erroralpha <- "Did you use the correct formula for `alpha`?"
ex() %>% check_object("alpha", undefined_msg = Erroralpha) %>% check_equal(incorrect_msg=Erroralpha)

success_msg("Well done! Some equate knowledge of 'environments' in R as being what is classically known as a 'power user'. It is not for everyone but it still nice to know that this powerful approach exists. You never know when you might need it." )
```
...
-   Use the [D()](https://www.rdocumentation.org/packages/mosaic/versions/0.14.4/topics/D) function to return an expression for the first and second derivatives with respect to $t$. Assign the results of `dMxtdt` and `d2Mxtdt2`, respectively.
::::

<br>

```{r ex="LDA1.3.1.4", type="hint", tut=TRUE}
Take some time to explore the online `R` documentation. In particular, take time to understand the required class of each argument.
```


```{r ex="LDA1.3.1.4", type="sample-code", tut=TRUE}
...
```{r ex="LDA1.3.1.4", type="solution", tut=TRUE}
dMxtdt <- D(expr = parse(text = "(1- theta * t) ^ -alpha"), name = 't')
...
```{r ex="LDA1.3.1.4", type="sct", tut=TRUE}
errordMxtd<-" Did you correctly specify the formula ?"
errordM2xtd<-" Did you correctly specify the formula ?"

ex() %>% check_object("dMxtdt", undefined_msg=errordMxtd) %>% check_equal(incorrect_msg=errordMxtd)
ex() %>% check_object("dM2xtdt", undefined_msg=errordM2xtd) %>% check_equal(incorrect_msg=errordM2xtd)

success_msg("Well done! The ability to parse expression is another somewhat advanced concept in `R`. Through this tutorial problem, we hope that you can see how this seemingly esoteric concept can be pretty handy.")
```
...
```{r ex="LDA1.3.1.5", type="hint", tut=TRUE}
Take some time to explore the online `R` documentation. In particular, take time to understand the required class of each argument.
```


```{r ex="LDA1.3.1.5", type="pre-exercise-code", tut=TRUE}
...
```{r ex="LDA1.3.1.5", type="sample-code", tut=TRUE}
...
```{r ex="LDA1.3.1.5", type="solution", tut=TRUE}
...
```{r ex="LDA1.3.1.5", type="sct", tut=TRUE}
errorEX<-" Did you correctly specify the formula ?"
errorEX2<-" Did you correctly specify the formula ?"

ex() %>% check_object("EX", undefined_msg=errorEX) %>% check_equal(incorrect_msg=errorEX)
ex() %>% check_object("EX2", undefined_msg=errorEX2) %>% check_equal(incorrect_msg=errorEX2)

success_msg("Well done! Being able to compute derivatives goes to the heart of calculus, the foundation of much of quantitative reasoning. We already know how important the moments of any distribution are -- this tutorial problem reinforces in an explicit way how a moment generating function can be used to 'generate' moments.")
```
...
### Exercise. Visualizing a Distribution using a Histogram
...
-   Use the [hist()](https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/hist) function to plot a histogram of `wisc_prop`.
...
-   Use the [str()](https://www.rdocumentation.org/packages/utils/versions/3.6.2/topics/str) function to review the structure of elements of the list returned by the histogram function hist().
:::: 

<br>

```{r ex="LDA1.3.2.1", type="hint", tut=TRUE}
Recall that, when you specify the `log()` in `R`, you get natural logarithmic values, not base 10 (common) nor base 2. 
```

```{r ex="LDA1.3.2.1", type="pre-exercise-code", tut=TRUE}
wisc_prop <- read.csv("https://raw.githubusercontent.com/OpenActTexts/LDACourse1/main/Data/Insample.csv", header=T, na.strings=c("."), stringsAsFactors = FALSE)
...
errorhist_data<-"Did you correctly specify the data?"

ex() %>% check_object("hist_data", undefined_msg=errorhist_data) %>% check_equal(incorrect_msg=errorhist_data)

success_msg("Well done!")
```

### Exercise. Visualizing the Gamma Distribution {#Ex:VisualGamma}
...
hist_data <- hist(x = ??, probability = ??, ylim=c(0,0.35))
...
hist_data <- hist(x = wisc_prop, probability = TRUE, ylim=c(0,0.35))
...
errorhist_data<-" Did you correctly specify data?"
ergamma_scale<-"Did you correctly specify the ratio of variance over mean?"
ergamma_shape<-"Did you correctly specify the ratio of mean over gamma_scale?"

ex() %>% check_object("hist_data", undefined_msg=errorhist_data) %>% check_equal(incorrect_msg=errorhist_data)
ex() %>% check_object("gamma_scale", undefined_msg=ergamma_scale) %>% check_equal(incorrect_msg=ergamma_scale)
ex() %>% check_object("gamma_shape", undefined_msg=ergamma_shape) %>% check_equal(incorrect_msg=ergamma_shape)

success_msg("Success! It is handy to be able to compare to a 'real' distribution (generated empirically by the data) to a 'theoretical' one (from a parametric family). It simply makes an audience much more comfortable with your analysis. Use this approach extensively, particularly when presenting results.")

```

### Exercise. Creating a Pareto Density Function {#Ex:CreateParetoDensityFct}
...
For this exercise we will write a `R` function to find the density (the pdf) of a random variable that follows a Pareto distribution. We will subsequently use this function in Exercise \@ref(Ex:MLEPareto).
...
You will also find a summary of the Pareto distribution in [Section 18.2.2 of  *Loss Data Analytics*](https://openacttexts.github.io/Loss-Data-Analytics/C-SummaryDistributions.html#two-parameter-distributions).
...
erdpareto<-"Did you correctly specify density function?"

ex() %>% check_object("dpareto", undefined_msg=erdpareto) %>% check_equal(incorrect_msg=erdpareto)

success_msg("Well done! A language like `R` is appealing to an actuarial audience because it is so easy to extend its basic capabilities through some pretty basic programming, such as creation of functions. Although analysts enjoy having scripts written in advance, often it is simply easier to build your own functions from basic principles. This has the real added advantage of you knowing exactly what the function is doing -- after all, you wrote it!")
```
...
You can get some theoretical background on transforming variables in  [Section 3.3 of  *Loss Data Analytics*](https://openacttexts.github.io/Loss-Data-Analytics/C-Severity.html#MethodsCreation).
...
erfx<-"Did you correctly specify shape and scale parameters?"
erfy<-"Did you correctly specify formula fx times x?"

ex() %>% check_object("fx", undefined_msg=erfx) %>% check_equal(incorrect_msg=erfx)
ex() %>% check_object("fy", undefined_msg=erfy) %>% check_equal(incorrect_msg=erfy)

success_msg("Success! You will see that it is common in insurance and risk management claims to encounter distributions that have 'long tails' - by this, we mean where there is a large probability of having an extremely large value. To handle this, it is common to take transforms, such as a log, to even start to think about how to represent the data using a model.")
```
...
eralpha<-"Did you correctly type values of given alphas?"
ertheta<-"Did you correctly type values of given thetas?"

ex() %>% check_object("alpha", undefined_msg=eralpha) %>% check_equal(incorrect_msg=eralpha)
ex() %>% check_object("theta", undefined_msg=ertheta) %>% check_equal(incorrect_msg=ertheta)

success_msg("Well done! Mixtures are complicated. They are part of the foundation material because, in many cases, they fit the data very, very well.")
```
...
* Calculate the expected value of the claims insured with a \$ 5 million policy limit using the [levlnorm()](https://www.rdocumentation.org/packages/actuar/versions/3.1-4/topics/LognormalMoments) function and then compare to the amounts insured from `claims`.
:::: 
...
erlevlnorm<- "Did you specify missing values?"
ermean<- "Did you specify missing values?"

ex() %>% check_function("levlnorm", not_called_msg=erlevlnorm) %>% check_result() %>% check_equal(incorrect_msg=erlevlnorm)
ex() %>% check_function("mean", not_called_msg=ermean) %>% check_result() %>% check_equal(incorrect_msg=ermean)
...
*  Calculate the expected value of the claims insured subject to a \$ 1 million deductible and a \$ 5 million policy limit using the [levlnorm()](https://www.rdocumentation.org/packages/actuar/versions/3.1-4/topics/LognormalMoments) and [plnorm()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/Lognormal)  function and the compare to the amounts you calculated in the prior step. Note that these are the 'ground-up' values meaning that they include the amounts within the deductible.
:::: 
...
ersum<- "Did you specify missing values?"
erinsured<-"Did you specify missing values?"
ermean<- "Did you specify missing values?"

ex() %>% check_object("insured", undefined_msg=erinsured) %>% check_equal(incorrect_msg=erinsured)

ex() %>% check_function("sum", not_called_msg=ersum) %>% check_result() %>% check_equal(incorrect_msg=ersum)
ex() %>% check_function("mean", not_called_msg=ermean) %>% check_result() %>% check_equal(incorrect_msg=ermean)
...
#Average Value
insured <- claims[claims > 1e6]
( M <- mean(pmin(5e6, insured) - 1e6) * (1 - 0.2) )
...
erinsured<-"Did you specify missing values?"
erM<- "Did you specify missing values?"

ex() %>% check_object("insured", undefined_msg=erinsured) %>% check_equal(incorrect_msg=erinsured)
ex() %>% check_object("M", undefined_msg=erM) %>% check_equal(incorrect_msg=erM)
...
### Exercise. Maximum Likelihood Estimation for a Pareto Distribution {#Ex:MLEPareto}
...
**Instructions** 
  ...
  -  Write a function to calculate the negative log-likelihood for the claim severity data, assuming that claim severity is Pareto distributed. The *pdf* function for a Pareto distribution [dpareto()], which was created in Exercise \@ref(Ex:CreateParetoDensityFct), can be used when finding the negative log-likelihood.
  -  Find the maximum likelihood estimates by using [optim()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/optim) to minimize the negative log-likelihood.
...
ernloglike<- "Did you specify correct formula?"
erlik_result<- "Did you specify correct formula: nloglike?"

ex() %>% check_object("nloglike", undefined_msg=ernloglike) %>% check_equal(incorrect_msg=ernloglike)
ex() %>% check_object("lik_result", undefined_msg=erlik_result) %>% check_equal(incorrect_msg=erlik_result)
...
For this exercise we calculate 95% confidence intervals for the parameters based on estimates that we found in Exercise \@ref(Ex:MLEPareto). The theory underpinning *mle* confidence intervals is described in [Section 15.3 of *Loss Data Analytics*](https://openacttexts.github.io/Loss-Data-Analytics/C-AppA.html#S:AppA:IE).
...
erInvHess<- "Did you use solve function for lik_result?"
ervPar<-"Did you use diagonal function for InvHess?"
erse<-"Did you use square root of vPar?"
ex() %>% check_object("InvHess", undefined_msg=erInvHess) %>% check_equal(incorrect_msg=erInvHess)
ex() %>% check_object("vPar", undefined_msg=ervPar) %>% check_equal(incorrect_msg=ervPar)
ex() %>% check_object("se", undefined_msg=erse) %>% check_equal(incorrect_msg=erse)
...
Here is a perspective on Chapter Three from **Laura Bosch,** a P \& C Actuarial Analyst with Willis Towers Watson. 

<center>

<iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/1660902/sp/166090200/embedIframeJs/uiconf_id/25717641/partner_id/1660902?iframeembed=true&playerId=kaltura_player&entry_id=1_chervi0p&flashvars[streamerType]=auto&amp;flashvars[localizationCode]=en_US&amp;flashvars[leadWithHTML5]=true&amp;flashvars[sideBarContainer.plugin]=true&amp;flashvars[sideBarContainer.position]=left&amp;flashvars[sideBarContainer.clickToClose]=true&amp;flashvars[chapters.plugin]=true&amp;flashvars[chapters.layout]=vertical&amp;flashvars[chapters.thumbnailRotator]=false&amp;flashvars[streamSelector.plugin]=true&amp;flashvars[EmbedPlayer.SpinnerTarget]=videoHolder&amp;flashvars[dualScreen.plugin]=true&amp;flashvars[Kaltura.addCrossoriginToIframe]=true&amp;&wid=1_uu9z6gbl" width="649" height="401" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *"  frameborder="0" title="Kaltura Player"></iframe>
...
-  **Chapter Maintainers**. Please contact Nii <okinean@appstate.edu> and/or Jed at <jfrees@bus.wisc.edu> for chapter comments and suggested improvements.
