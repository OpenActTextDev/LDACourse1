...
*  There is no `base-R` function to calculate the coefficient of skewness. There are several approaches to calculate skewness if you explore the [help()](https://www.rdocumentation.org/packages/e1071/versions/1.7-4/topics/skewness) for the `type` argument of the `skewness` function from the [e1071](https://www.rdocumentation.org/packages/e1071/versions/1.7-7) package.  We will not use this package, but instead we will we calculate the equivalent of `type = 3` skewness using `var` in the denominator.
:::: 
...
#Summary of Data
s1<-summary(anscombe$y1) #Example for the first data set in  Anscombe's quartet
s2<-summary(anscombe$??)
s3<-summary(anscombe$??)
s4<-summary(anscombe$??)
s1
...
#Vector of means
c(mean(anscombe$y1), mean(anscombe$??), mean(anscombe$??),
  mean(anscombe$??))

#M<-c(mean(anscombe$y1), mean(anscombe$y2), mean(anscombe$y3),  mean(anscombe$y4))
M1=mean(anscombe$y1)
M2=mean(anscombe$y2)
M3=mean(anscombe$y3)
M4=mean(anscombe$y4)
...
#Vector of variances
c(var(anscombe$y1), var(anscombe$??), var(anscombe$??),
  var(anscombe$??))
V1<-var(anscombe$y1)
V2<-var(anscombe$y2)
V3<-var(anscombe$y3)
...
# Skewness of data y1
y1_skew <- mean((anscombe$y1 - mean(anscombe$??))^3) / sd(anscombe$y1) ^ 3
y1_skew
...
#Summary of Data
s1<-summary(anscombe$y1) #Example for the first data set in  Anscombe's quartet
s2<-summary(anscombe$y2)
s3<-summary(anscombe$y3)
s4<-summary(anscombe$y4)
s1
s2
s3
s4
...
Error2a <- "Did you call the function `mean` for y1?"
Error2b <- "Did you call the function `mean` for y2?"
Error2c <- "Did you call the function `mean` for y3?"
Error2d <- "Did you call the function `mean` for y4?"

Error3a <- "Did you call the function `var`for y1?"
Error3b <- "Did you call the function `var` for y2?"
Error3c <- "Did you call the function `var` for y3?"
Error3d <- "Did you call the function `var` for y4?"



#Error4 <- "Did you assign the object using correct skewness formula`?  # we disable because of data camp error 


ex() %>% check_object("s1", undefined_msg = Error1a) %>% check_equal(incorrect_msg=Error1a)
ex() %>% check_object("s2", undefined_msg = Error1b) %>% check_equal(incorrect_msg=Error1b)
ex() %>% check_object("s3", undefined_msg = Error1c) %>% check_equal(incorrect_msg=Error1c)
ex() %>% check_object("s4", undefined_msg = Error1d) %>% check_equal(incorrect_msg=Error1d)

ex() %>% check_object("M1", undefined_msg = Error2a) %>% check_equal(incorrect_msg=Error2a)
ex() %>% check_object("M2", undefined_msg = Error2b) %>% check_equal(incorrect_msg=Error2b)
ex() %>% check_object("M3", undefined_msg = Error2c) %>% check_equal(incorrect_msg=Error2c)
ex() %>% check_object("M4", undefined_msg = Error2d) %>% check_equal(incorrect_msg=Error2d)

ex() %>% check_object("V1", undefined_msg = Error3a) %>% check_equal(incorrect_msg=Error3a)
ex() %>% check_object("V2", undefined_msg = Error3b) %>% check_equal(incorrect_msg=Error3b)
ex() %>% check_object("V2", undefined_msg = Error3c) %>% check_equal(incorrect_msg=Error3c)
ex() %>% check_object("V4", undefined_msg = Error3d) %>% check_equal(incorrect_msg=Error3d)
...
The [boxplot()](https://www.rdocumentation.org/packages/graphics/versions/3.6.2/topics/boxplot) function produces box-and-whisker plots of grouped values. The boxplot() function returns certain statistics and produces a plot as a side effect. Explore the documentation  to understand the values returned. In particular review the information returned by the `stats` attribute of this object.
...
#Errormgf <- "Did you use the correct formula for mgf?"
#ex() %>% check_object("mgf", undefined_msg = Errormgf) %>% check_equal(incorrect_msg=Errormgf)
...
```{r ex="LDA1.3.1.3.3", type="pre-exercise-code", tut=TRUE}
...
* Calculate the expected insured value of the claims subject to a \$1 million deductible and 20% coinsurance and a \$5 million policy limit using the [levlnorm()](https://www.rdocumentation.org/packages/actuar/versions/3.1-4/topics/LognormalMoments) and [plnorm()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/Lognormal) function and the compare to the amounts insured from `claims`. In contrast to the prior exercise, calculate the insured value of the claim rather than the ground-up value.
:::: 
