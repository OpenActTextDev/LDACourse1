...
```{r eval = FALSE}
FUN2 <- function(x) { c(Minimum=min(x), Median=median(x), 
                        Mean=mean(x),  Maximum=max(x) ) }
Table2 <- rbind(FUN2(Insample$Freq), FUN2(Insample$yAvg),
                FUN2(Insample$Deduct), FUN2(Insample$BCcov/1000))
rownames(Table2) <- c("Claim Frequency", "Claim Severity", 
                      "Deductible", "Coverage (000's)")
round(Table2, digits = 0)
```
